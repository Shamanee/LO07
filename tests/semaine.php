...
<table class="calendar__table__heure">
    ...
    <?php for ($heure = 0; $heure < 24; $heure++): ?>
        <tr class="">
            <td class="calendar__heure" style="width:5%;"><?= $heure ?></td>
            <?php
            foreach ($week->days as $k => $day):
                $start3 = clone $start;
                ...
                <td class="<?php
                        $nounouId = $_SESSION['id'];
                            ...
                            //var_dump($jour);
                            $sql1 = "SELECT Debut, Fin FROM disponibilite WHERE utilisateur_id=$nounouId AND jour=$jour";
                            $req1 = $bdd->query($sql1);
                            $res1 = $req1->fetchAll();
                            //var_dump($res1);
                            if (!empty($res1)) {
                                ...
                                $date_m = $date->format("{$h}:i:s");
                                //var_dump($date_m);
                                //var_dump($res1[0]['Fin']);
                                if ($date_m < $res1[0]['Fin'] && $date_m >= $res1[0]['Debut']) {
                                    echo 'test';
                                }
                            }
                        ?>">
                        ...
                        <div class="">
                            ...
                            if ($date_modif < $event['fin_datetime'] && $date_modif >= $event['debut_datetime']):
                                ...
                                <div style="background-color: red">
                                    <a style="color:white;"href="event.php?id=<?= $event['id']; ?>"><?= $res['nom'] ?></a>
                                </div>
                                <?php
                            endif;
                            ?>
